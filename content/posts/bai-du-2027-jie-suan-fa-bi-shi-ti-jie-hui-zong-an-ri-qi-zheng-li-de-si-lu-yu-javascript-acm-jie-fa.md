...
为什么可以这样改写？`F(y)` 包含 `0..x-1` 和 `x..y` 两段。再异或一次 `F(x-1)` 后，前一段中的每个数都出现两次，而 `a xor a=0`，所以它们全部抵消，只留下 `x..y`：

```text
x xor (x+1) xor ... xor y = F(y) xor F(x-1)
```
...
在等式两边同时异或 `F(x-1)`，它也会在左边抵消，于是得到：

```text
F(y) = z xor F(x-1)
```

代码把右边这个已经确定的值命名为 `target`：

```javascript
const target = z ^ prefixXor(x - 1)
```

现在问题已经变成：**寻找最小的 `y >= x`，使 `F(y) === target`。**
...
把表格中的 `n` 换成正在寻找的 `y`，逐行反推：

| `y mod 4` | `F(y)` | 要满足 `F(y)=target`，`y` 必须怎样取 |
| --- | --- | --- |
| 0 | `y` | 只能令 `y=target`，再检查它是否真的模 4 等于 0 |
| 1 | `1` | 只有 `target=1` 才可能；寻找不小于 `x` 的最近一个模 4 等于 1 的数 |
| 2 | `y+1` | 只能令 `y=target-1`，再检查它是否真的模 4 等于 2 |
| 3 | `0` | 只有 `target=0` 才可能；寻找不小于 `x` 的最近一个模 4 等于 3 的数 |

第 1、3 行都能直接反解出一个具体数字，所以加入：

```text
target、target-1
```

第 2、4 行得到的是常量 1 和 0，无法从 `target` 直接反解出唯一的 `y`。但是任意连续四个整数的模 4 余数一定分别是 0、1、2、3，所以 `x,x+1,x+2,x+3` 中必然包含最近的 `y mod 4 = 1` 和最近的 `y mod 4 = 3`。因此再加入：

```text
x、x+1、x+2、x+3
```

两部分合起来，所有可能产生最小答案的数都在下面六个候选中：

```javascript
const candidates = [x, x + 1, x + 2, x + 3, target - 1, target]
```

候选中允许有重复值，也允许有不合法的值。循环里的两个条件负责过滤：

```javascript
if (y >= x && prefixXor(y) === target)
```

- `y >= x` 保证右端点没有跑到区间左端点之前；
- `prefixXor(y) === target` 同时验证数值和模 4 条件，因此不需要分别讨论四种余数；
- `Math.min` 从所有合法候选中留下最小的 `y`。
...
逐个验证后只有 `y=4` 满足 `F(4)=4`，所以答案是 4。代回原式也有 `1 xor 2 xor 3 xor 4=4`。

第二组专门观察“常量 1”的情况：`x=6,z=0`。
...
`target` 和 `target-1` 都小于 `x`，但 `x..x+3` 中的 9 满足 `9 mod 4=1`，所以 `F(9)=1=target`，答案是 9。此时原区间也确实满足 `6 xor 7 xor 8 xor 9=0`。
